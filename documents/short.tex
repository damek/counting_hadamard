...
I'mDe~Launey and Levin~\cite{DL10} introduced a Fourier-analytic framework that answers this question for large~\(t\). Their approach encodes the row-orthogonality conditions for an \(n\times t\) matrix through the \(d=\binom{n}{2}\) pairwise products \(y_iy_j\): writing \(Z(y)=(y_iy_j)_{1\le i<j\le n}\in\{\pm1\}^d\), a matrix \(Y=[y^{(1)}\cdots y^{(t)}]\) with \(\pm1\) entries is partial Hadamard if and only if \(\sum_{r=1}^t Z(y^{(r)})=0\); see Figure~\ref{fig:walk}. Counting partial Hadamard matrices therefore reduces to counting returns to the origin of the random walk \(S_t=\sum_{r=1}^t Z(\xi^{(r)})\) on~\(\mathbb{Z}^d\), where \(\xi^{(1)},\xi^{(2)},\dots\) are i.i.d.\ uniform on~\(\{\pm1\}^n\). If \(N_{n,t}\) denotes the number of \(n\times t\) partial Hadamard matrices,
